...
- (void)sendVariableLengthRPC
{
    ...
    /*
     和服务器交互的格式:   @@##|报文长度|session|图片内容|$$%%
     **/
    //图片数据
    
    NSString* tempStr = @"/img1300/img100.jpg";
   // NSData * data = [tempStr dataUsingEncoding:NSUTF8StringEncoding];
    
    //图片长度
    //NSUInteger dataLength = [data length];
    //NSInteger dataLength = [tempStr length];
    NSString * regularStr    = [NSString stringWithFormat:@"@@##"];
    //NSString * regularStr1   = [NSString stringWithFormat:@"|%d|",10086];
    NSString * regularStr2   = [NSString stringWithFormat:@"$$%%%%"];
    //@@##|报文长度|session|图片内容|$$%%

   // NSInteger tempSendDataLength = [regularStr length] + [regularStr2 length] + dataLength;
   // NSLog(@"Length====:[regularStr length]:%lu====[regularStr1 length]:%lu=====dataLength:%lu",(unsigned long)[regularStr length],(unsigned long)[regularStr2 length],(unsigned long)dataLength);
    
   // NSLog(@"tempSendDataLength:%ld",(long)tempSendDataLength);
    /*
    if (tempSendDataLength > 0 && tempSendDataLength <= 9)
    {
        tempSendDataLength += 1;
    }
    if (tempSendDataLength < 99 && tempSendDataLength >=10)
    {
        tempSendDataLength += 2;
    }
    if (tempSendDataLength >= 100 && tempSendDataLength <= 999)
    {
        tempSendDataLength += 3;
    }
    if (tempSendDataLength >= 1000 && tempSendDataLength <= 9999)
    {
        tempSendDataLength += 4;
    }
    if (tempSendDataLength >= 10000 && tempSendDataLength <= 99999)
    {
        tempSendDataLength += 5;
    }
*/
    
    NSString * sendStrData = [NSString stringWithFormat:@"%@%@%@",regularStr,tempStr,regularStr2];
    //NSData * data = [sendStrData dataUsingEncoding:NSUTF8StringEncoding];
    //NSString * sendStrData = [NSString stringWithFormat:@"%@%ld%@%@%@",regularStr,(long)tempSendDataLength,regularStr1,data,regularStr2];
   // NSData * sendStrData = [NSData dataWithData:data];
   // NSData * sendStrData =[RHSocketUtils dataFromHexString:sendStrData1];
    /*
    NSData *sendData = [sendStrData dataUsingEncoding:NSUnicodeStringEncoding];
    
    NSMutableData *allData = [NSMutableData dataWithCapacity:0];
    
    [allData replaceBytesInRange:NSMakeRange(0, sendData.length) withBytes:sendData.bytes];
    */
    NSLog(@"sendStrData:%@",sendStrData);
    
    RHSocketPacketRequest *req = [[RHSocketPacketRequest alloc] init];
    req.object = sendStrData;//sendStrData
    req.pid = 97;
    /*
    RHSocketDelimiterEncoder *encoder = [[RHSocketDelimiterEncoder alloc] init];
    encoder.delimiterData = [RHSocketUtils dataFromNormalString:@"$$%%" byteCount:2];
    [RHSocketService sharedInstance].encoder = encoder;
    */
    //RHSocketDelimiterDecoder *decoder = [[RHSocketDelimiterDecoder alloc] init];
    //decoder.delimiterData = [RHSocketUtils dataFromNormalString:@"$$%%%%" byteCount:2];
